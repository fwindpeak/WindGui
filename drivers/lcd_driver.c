...
void lcd_clear()
{
    ...
    for (index=0; index<(LCD_LONG*LCD_SHORT); index++)
    {
        write_data(LCD_BKCOLOR);
...
/*******************************************************
  ...
  *    2 
  *******************************************************/
void lcd_set_direction(unsigned int direction)
{
    switch(direction)
    {
    case 0:
        ...
        lcd.width=LCD_SHORT;
        lcd.height=LCD_LONG;
        write_reg(0x03,0x1000);	//
        break;
    case 1:
        lcd.direction=1;
        lcd.width=LCD_LONG;
        lcd.height=LCD_SHORT;
...
//设置背景颜色
void lcd_set_bkcolor(unsigned int color)
{
    LCD_BKCOLOR = color;
}
//获取背景颜色
unsigned int lcd_get_bkcolor()
{
    return LCD_BKCOLOR;
}
//设置前景颜色
void lcd_set_forcolor(unsigned int color)
{
    LCD_FORCOLOR = color;
}
//获取前景颜色
unsigned int lcd_get_forcolor()
{
    return LCD_FORCOLOR;
}
